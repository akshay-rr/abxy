...
class RewardsController:
    ...
    def calculateRepeatBonus(self,taskEntry, taskObj):
        if taskObj:
            repeatBonusObject = self.taskDatabase.getRepeatBonusByIDandUID(taskObj.repeat_bonus_id, taskEntry.uid)
            # Calculation
            ans=0
            return ans
        return None
    ...
    def calculateFocusBonus(self,taskEntry, taskObj):
        if taskObj:
            focusBonusObject = self.taskDatabase.getFocusBonusByIDandUID(taskObj.focus_bonus_id, taskEntry.uid)
            # Calculation
            ans=0
            return ans
        return None
